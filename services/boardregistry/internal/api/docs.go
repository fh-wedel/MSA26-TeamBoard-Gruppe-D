...
// /board-registry so the existing Traefik router can forward them without a
// prefix collision with the plugin service. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/board-registry/openapi.yaml", serveOpenAPISpec)
	r.Get("/board-registry/docs", serveDocsUI("TeamBoard Board Registry API", "/board-registry/openapi.yaml"))
}
