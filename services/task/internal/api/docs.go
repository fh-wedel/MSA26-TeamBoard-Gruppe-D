...
// /task so the existing Traefik router can forward them without a prefix
// collision with the other services. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/task/openapi.yaml", serveOpenAPISpec)
	r.Get("/task/docs", serveDocsUI("TeamBoard Task API", "/task/openapi.yaml"))
}
