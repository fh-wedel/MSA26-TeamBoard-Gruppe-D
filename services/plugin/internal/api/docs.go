...
// /plugin so the existing Traefik router can forward them without a prefix
// collision with the board-registry service. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/plugin/openapi.yaml", serveOpenAPISpec)
	r.Get("/plugin/docs", serveDocsUI("TeamBoard Plugin / Webhook API", "/plugin/openapi.yaml"))
}
