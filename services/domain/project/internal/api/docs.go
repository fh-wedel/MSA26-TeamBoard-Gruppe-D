...
// /project so the existing Traefik router can forward them without a prefix
// collision with the other services. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/project/openapi.yaml", serveOpenAPISpec)
	r.Get("/project/docs", serveDocsUI("TeamBoard Project API", "/project/openapi.yaml"))
}
