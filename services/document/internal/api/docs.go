...
// /document so the existing Traefik router can forward them without a prefix
// collision with the other services. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/document/openapi.yaml", serveOpenAPISpec)
	r.Get("/document/docs", serveDocsUI("TeamBoard Document API", "/document/openapi.yaml"))
}
