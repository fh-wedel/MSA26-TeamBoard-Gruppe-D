...
// /notification so the existing Traefik router can forward them without a
// prefix collision with the other services. No auth — the docs are public.
func mountDocs(r chi.Router) {
	r.Get("/notification/openapi.yaml", serveOpenAPISpec)
	r.Get("/notification/docs", serveDocsUI("TeamBoard Notification API", "/notification/openapi.yaml"))
}
