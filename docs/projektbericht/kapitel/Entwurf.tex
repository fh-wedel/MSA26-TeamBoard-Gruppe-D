...
Dieses Kapitel stellt die Architektur von TeamBoard vor. Der Schwerpunkt liegt auf den Entwurfsentscheidungen und den Abwägungen, die zu ihnen geführt haben, und weniger auf einer vollständigen Aufzählung aller Funktionen.
...
Die Architektur löst die zwölf verbindlichen Prinzipien aus \vref{subsec:nichtfunktionale-anforderungen} ein. Die folgenden Abschnitte diskutieren die wichtigsten der zugrunde liegenden Entscheidungen.

\subsection{Microservices}

Die Aufgabenstellung fordert ausdrücklich ein komponentenbasiertes System mit unabhängig skalierbaren Diensten. Ein modularer Monolith wäre in der Entwicklung und im Betrieb einfacher gewesen und hätte verteilte Transaktionen vermieden. Er hätte jedoch die geforderte unabhängige Skalierung und die klare fachliche Trennung nur eingeschränkt abgebildet. Die Entscheidung fiel daher auf einen Schnitt in eigenständige Dienste \cite{newman2021microservices}. Der Preis dafür ist ein höherer Betriebsaufwand und die Notwendigkeit, Konsistenz über Dienstgrenzen hinweg aktiv zu behandeln.

Der Zuschnitt orientiert sich an Domain-Driven Design \cite{evans2003ddd}. Jeder Dienst bildet einen fachlichen Bounded Context mit eigener Sprache ab und folgt dem MVP-Schnitt aus \vref{sec:interpretation}.
...
Der Task-Service verwaltet Aufgaben innerhalb eines Boards mit Statusübergängen, Zuweisungen, Fälligkeiten, Kommentaren und Anhang-Referenzen. Er führt einen Audit-Trail über Änderungen. Den fachlichen Status einer Spalte übernimmt er aus den Status-Angaben, die über Ereignisse aus dem Board-Kontext eintreffen. Vor schreibenden Operationen prüft er die Berechtigung beim Project-Service.
...
\vref{fig:ablauf-event} stellt diesen asynchronen Ablauf als Sequenz dar.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}lX@{}}
    ...
    \path{docs/decisions/*.md}            & ADRs zu Board Registry \\
    \path{docs/services/*.md}                  & Detail-Design je Dienst mit Datenmodell und Schnittstellen \\
    \path{docs/specifications/*.md}   & Code Guidelines, Deployment und weitere Spezifikationen \\
    \bottomrule
  \end{tabularx}
\end{table}
