...
\subsection{Microservices statt Monolith}
...
\subsection{Asynchrone Ereignisse statt durchgehend synchroner Aufrufe}
...
\subsection{Database-per-Service statt geteilter Datenbank}
...
\subsection{Outbox-Pattern statt direktem Publizieren}
...
\subsection{Zero-Trust statt Authentifizierung am Gateway}
...
\ausarbeiten{Vertiefte Abwägung zu einer weiteren Entscheidung, zum Beispiel Wahl von PostgreSQL gegenüber getrennten Datenspeichern je Dienst oder Wahl von Traefik als Gateway. Bei Bedarf mit kurzer Begruendung ergaenzen.}
...
\ausarbeiten{SSRF-Schutz der ausgehenden Aufrufe knapp darstellen, siehe plugin.md Abschnitt 10.}
...
Der MCP-Server bindet Sprachmodell-Clients wie Claude an das System an. Er ist kein Domain-Service, sondern ein Protokoll-Adapter nach dem Muster eines Backend-for-Frontend. Er übersetzt eingehende Tool-Aufrufe des Model-Context-Protocol in die internen REST-Verträge und besitzt daher keine eigene Datenbank, keine Ereignisse und keinen Zustand. Er authentifiziert nicht selbst, sondern reicht ein Personal Access Token an das Gateway durch und sieht damit genau das, was der Eigentümer des Tokens sehen darf. Der Nutzen dieses Zuschnitts liegt darin, dass ein neuer Client-Typ neben dem Web-Frontend angebunden wird, ohne dass ein Domain-Service den fremden Client oder sein Protokoll kennen muss. Die Dienste bleiben von der Anbindung unberührt.

\subsection{API-Gateway und Frontend}
\label{sub:gateway-frontend}

Das API-Gateway auf Basis von Traefik ist der einzige Eintrittspunkt. Es übernimmt Routing, Rate-Limiting, CORS und Sicherheits-Header und terminiert im Betrieb TLS. Geschäftslogik und Token-Prüfung liegen bewusst nicht im Gateway. Das Frontend ist eine React-basierte Single-Page-Anwendung. Es enthält die bereits beschriebene View-Registry, die die Darstellung eines Boards anhand der Presentation-Spezifikation seines Typs wählt.

\ausarbeiten{Falls gewuenscht, kurze Notiz zum konkreten Frontend-Aufbau und den wichtigsten Views ergaenzen.}
